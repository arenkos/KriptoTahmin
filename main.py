...
def backtest_strategy(timeframe, df):
    ...
    atr_multipliers = [x / 10 for x in range(10, 51, 5)]  # 1.0'dan 5.0'a 0.5 artışla
    ...
    while leverage <= 50:
        ...
        while stop_percentage <= 5:
            ...
            for i in range(3, len(df)):
                try:
                    ...
                    if son_kapanis > son_supertrend and onceki_kapanis < onceki_supertrend:
                        ...
                        for j in range(i, len(df)):  # Tüm mumları kontrol et
                            # Stop loss kontrolü
                            if float(df["low"].iloc[j]) <= giris_fiyati * (1 - stop_percentage/100):
                                kar_zarar = -stop_percentage
                                bakiye = bakiye * (1 + kar_zarar/100 * leverage)
                                basarisiz += 1
                                trades.append(kar_zarar * leverage)
                                break

                            # Satış sinyali kontrolü
                            if close_array[j-2] < supertrend[j-2] and close_array[j-3] > supertrend[j-3]:
                                kar_zarar = (float(df["open"].iloc[j]) - giris_fiyati) / giris_fiyati * 100
                                bakiye = bakiye * (1 + kar_zarar/100 * leverage)
                                if kar_zarar > 0:
                                    basarili += 1
                                else:
                    ...
                    # Short pozisyon sinyali
                        ...
                        for j in range(i, len(df)):  # Tüm mumları kontrol et
                            # Stop loss kontrolü
                            if float(df["high"].iloc[j]) >= giris_fiyati * (1 + stop_percentage/100):
                                kar_zarar = -stop_percentage
                                bakiye = bakiye * (1 + kar_zarar/100 * leverage)
                                basarisiz += 1
                                trades.append(kar_zarar * leverage)
                            ...
                            if close_array[j-2] > supertrend[j-2] and close_array[j-3] < supertrend[j-3]:
                                kar_zarar = (giris_fiyati - float(df["open"].iloc[j])) / giris_fiyati * 100
                                bakiye = bakiye * (1 + kar_zarar/100 * leverage)
                                if kar_zarar > 0:
                                    basarili += 1
                                else:
            ...
            if total_trades > 0:
                ...
                profit_rate = (bakiye - 100.0)

                # En iyi sonucu güncelle
                if profit_rate > best_result['profit_rate']:
                    best_result.update({
                        'leverage': leverage,
...
def analyze_symbol(symbol, timeframe, exchange):
    ...
    try:
        ohlcv_service = OHLCVService(exchange)
        start_date = datetime.utcnow() - timedelta(days=365)  # 1 yıllık veri
        end_date = datetime.utcnow()
